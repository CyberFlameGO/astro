...
async function load(config: RuntimeConfig, rawPathname: string | undefined): Promise<LoadResult> {
  ...
  try {
    const ssr = (await viteServer.ssrLoadModule(fileURLToPath(new URL('./ssr.js', import.meta.url)))).default;
    const compileOptions: CompileOptions = { astroConfig: config.astroConfig, logging, mode: config.mode };
    const mod = await ssr.load(searchResult.location, { compileOptions });
...
async function createVite(astroConfig: AstroConfig, options: CreateViteOptions) {
...
export async function createRuntime(astroConfig: AstroConfig, { mode, logging }: RuntimeOptions): Promise<AstroRuntime> {
  ...
  const [viteServer, staticServer] = await Promise.all([createVite(astroConfig, { mode }), createStaticServer(astroConfig)]);
